...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
		...
		\date{\today}\par
		\vfill
		%\includegraphics[scale=1]{couple.pdf}
		\endgroup}
	...
	<xsl:template match="CUT" >\color{cut} <xsl:apply-templates/>\color{black}</xsl:template>
...
<xsl:template match="speech/stagedir">
	\begin{onehalfspacing}\direct{<xsl:value-of select="." />}\end{onehalfspacing}

</xsl:template>



<xsl:template match="line/stagedir">{\em(<xsl:value-of select="./text()" />)}</xsl:template>
